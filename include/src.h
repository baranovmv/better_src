...
typedef enum { MONO, STEREO } SrcNumChannels_t;

SRC_API src_t* src_open(SrcProfile_t profile, SrcNumChannels_t nchannels, unsigned int in_fs, unsigned int out_fs);
SRC_API int src_set_scale(src_t* src, float coeff);
SRC_API int src_push_samples(src_t* src, const float* samples, unsigned int nsamples);
...
SRC_API const char* src_profile_to_str(const SrcProfile_t profile);
